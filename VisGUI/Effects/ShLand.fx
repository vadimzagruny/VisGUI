...
texture BlendMaskMap : BlendMaskMap<			
	string name		= "seafloor.dds"; 
	string UIName	= "Mask Texture  ";
>;


sampler SamplerBlendMask = sampler_state			//SMASK
{
...
sampler SamplerDiffuse1 = sampler_state			//S1
{
	texture = <DiffuseMap1>;
	minfilter  = ANISOTROPIC;
	mipfilter  = ANISOTROPIC;
...
sampler SamplerDiffuse2 = sampler_state			//S2
{
	texture = <DiffuseMap2>;
	minfilter  = ANISOTROPIC;
	mipfilter  = ANISOTROPIC;
...
sampler SamplerDiffuse3 = sampler_state			//S3
{
	texture = <DiffuseMap3>;
	minfilter  = ANISOTROPIC;
	mipfilter  = ANISOTROPIC;
		...
		out float4 ocolor	: COLOR )
{
	...
	float4 tex_color2 = tex2D( SamplerDiffuse1, texcrd1 );
	float4 tex_color3 = tex2D( SamplerDiffuse2, texcrd1 );
	float4 tex_color4 = tex2D( SamplerDiffuse3, texcrd1 );
	
	float4 tex_color5	= tex2D( SamplerAmbient,	texcrd2 );
	float4 tex_mask		= tex2D( SamplerBlendMask,	texcrd2 );
	...
	float4 TexColor = tex_color1 ;
	DifColor += GetAngleIntensity(w_norm,iPos);

	TexColor.rgb = MapBlend4(tex_color1,tex_mask.r,tex_color2,tex_mask.g,tex_color3,tex_mask.b,tex_color4,tex_mask.a);

	Color  =  TexColor /** tex_color5*/ * DifColor ;
	Color += SpecColor;
